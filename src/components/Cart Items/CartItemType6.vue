<template lang="">
    <div class="card abw_cart_item_name">
        <div class="card-header" id="headingOne">
           ...
           <div v-for="pack of cart_item_data.packages" :key="pack.id" class="row" style="margin-top: 4px;margin-bottom: 4px;">
                        <div class="col-md-8">
                            <p class="text-left abw_cart_item_price_label"><small>{{ pack.name}} x {{ pack.qty }}</small></p>
                        </div>
                        <div class="col-md-4 abw_cart_item_base_price">
                            <p class="text-right">
                                
                                
                                <small>
                                <strong>{{this.$store.getters.getCurrency}}  {{ pack.price[pack.qty] ?? (pack.price[1] * pack.qty) }}</strong>
                                </small>
                            </p>
                        </div>
  ...
  computed: {
    ...
    subTotal() {
      ...
      this.cart_item_data.packages.forEach((element) => {
        subTotal = subTotal + (element.price[element.qty] ?? (element.price[1] * element.qty));
      });

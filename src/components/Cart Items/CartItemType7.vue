<template lang="">
    <div class="card">
        <div class="card-header" id="headingOne">
           ...
           <div v-for="sess of cart_item_data.sessions" :key="sess.id" class="row" style="margin-top: 4px;margin-bottom: 4px;">
                        <div class="col-md-8">
                            <p class="text-left abw_cart_item_price_label"><small>{{ sess.name}} </small></p>
                        </div>
                        <div class="col-md-4">
                            <p class="text-right abw_cart_item_base_price">
                                <small v-for="(price , index) of sess.price" :key="price">
                                            <td v-if="index == 1">
  ...
  computed: {
    ...
    subTotal() {
      ...
      this.cart_item_data.sessions.forEach((element) => {
        subTotal = subTotal + parseFloat(element.price[1]);
      });
